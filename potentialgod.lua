-- ====================================================================
-- [DIX] FINAL SCRIPT V27.0 (VISUALS TAB + ESP)
-- All Combat features preserved. Added new VISUALS tab with Player ESP.
-- ====================================================================
...
local Drawing = pcall(function() return Drawing end) and Drawing or nil -- Check if Drawing is available
...
local ESPColor = Color3.fromRGB(255, 255, 0) -- Yellow
local ESPConnection = nil
local ESPDrawings = {} -- To store and manage Drawing objects

-- ====================================================================
-- [DIX FOV MODULE V27.0] - VISUALIZATION (No changes)
-- ====================================================================
...
local function SetupFOVVisuals()
    if Drawing then
        ...
        ScreenGui.Name = "DIX_FOV_ROOT_V27"
        ScreenGui.IgnoreGuiInset = true
        ScreenGui.ZIndexBehavior = Enum.ZIndexBehavior.Global
        ScreenGui.Parent = PlayerGui

        FOV_Visual_Element = Instance.new("Frame")
        FOV_Visual_Element.Name = "DIX_FOV_Frame_V27_GUI" 
...
-- ====================================================================
-- [Aimbot Core Functions] (Logic unchanged from V26.0)
-- ====================================================================
...
-- ====================================================================
-- [Hitbox Expander Core Functions] (Logic unchanged from V26.0)
-- ====================================================================
...
local function ApplyHitboxExpansion(Player)
    ...
    for _, PartName in ipairs(Hitbox_Parts_To_Change) do
        ...
        if Part and Part:IsA("BasePart") then
            ...
            if OriginalSize and Part.Size ~= OriginalSize * Hitbox_Multiplier then
...
local function DrawPlayerESP(Player)
    ...
    local ScreenPos, IsOnScreen = Camera:WorldToScreenPoint(RootPart.Position)
    if not IsOnScreen then return end

    local PosX, PosY = ScreenPos.X, ScreenPos.Y
    ...
    if IsESPNameEnabled and Drawing then
        ...
        NameText.Position = Vector2.new(PosX, PosY - 20)
    ...
    if IsESPDistanceEnabled and Drawing then
        ...
        DistanceText.Position = Vector2.new(PosX, PosY + 5)
        DistanceText.Color = ESPColor
        DistanceText.Visible = true
    end

    -- 2D Box (Box Drawing - simplified for fast rendering)
    if IsESPBoxEnabled and Drawing then
        ...
        -- Simplified 2D Box size calculation (adjust as needed for game models)
        local boxHeight = 100
        local boxWidth = 50
        
        Box.Position = Vector2.new(PosX - boxWidth / 2, PosY - boxHeight / 2)
...
local function ESPLoop()
    ...
    -- Hide all drawings first (prevents stale drawings if player leaves)
    for _, drawing in pairs(ESPDrawings) do
...
if WindUi then
    ...
    -- ================================================================
    -- VISUALS SECTION 1: ESP Settings (NEW)
    -- ================================================================
    ...
    print("DIX HUB V27.0 launched. New 'VISUALS' tab with ESP added.")
end
