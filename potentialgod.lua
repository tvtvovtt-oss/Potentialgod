...
local function DrawPlayerInfo(Player)
    ...
    local Y_Offset_Start = BottomY + 5 -- Начало отрисовки через 5 пикселей под моделью
    ...
    if IsESPNameEnabled and Drawing then
        ...
        NameText.Position = Vector2.new(CenterX, Y_Offset_Start) -- Ник
...
if WindUi and WindUi.CreateWindow then 
    ...
    -- Target Selector (Toggles) - СЕКЦИЯ, КОТОРАЯ НЕ ПРОПАЛА
