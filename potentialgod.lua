-- ====================================================================
-- [DIX] FINAL SCRIPT V40.1 (FOV Circle Fix + Mobile Stable)
-- FIX: Added Draw FOV Circle functionality.
-- FIX: Reverted GUI structure to V33.0 stable base.
-- FIX: Name and Distance ESP is now positioned BELOW the player model.
-- ADD: Mobile Silent Aim trigger (Touch/M1 + Virtual Jump Button).
-- ====================================================================
...
local ESPDrawings = {} 
local ESPHighlights = {} 
local IsFOVDrawEnabled = true -- НОВОЕ: Включить/Выключить отрисовку круга FOV
local FOVDrawing = nil -- НОВОЕ: Объект для отрисовки круга FOV
...
local function DrawFOVCircle()
    ...
    -- R = (tan(FOV/2) / tan(Camera.FieldOfView/2)) * (ScreenHeight / 2)
...
local function AimFunction()
    if not Camera or not LocalPlayer.Character or (not IsAimbotEnabled and not IsSilentAimEnabled) then 
        CurrentTarget = nil 
        if FOVDrawing then FOVDrawing:Remove() FOVDrawing = nil end -- Очистка при выключении
        return 
    end
    ...
    DrawFOVCircle() -- НОВОЕ: Отрисовка круга FOV
end
...
local function StopAiming()
    ...
    if FOVDrawing then FOVDrawing:Remove() FOVDrawing = nil end -- Очистка
end
...
-- MODIFIED: Name and Distance are now BELOW the model
local function DrawPlayerInfo(Player)
    ...
    -- Определяем самую нижнюю точку модели на экране для начала отрисовки
...
if WindUi and WindUi.CreateWindow then 
    local Window = WindUi:CreateWindow({
        Title = "DIX HUB V40.1 (Mobile Stable)",
        Author = "by Dixyi",
    ...
    -- Tags
    Window:Tag({ Title = "V40.1", Icon = "mobile", Color = Color3.fromHex("#6b31ff") })
    ...
    -- НОВОЕ: Переключатель для отрисовки круга FOV
    AimSection:Toggle({
        Flag = "DrawFOVCircle",
    ...
    Window:OnClose(function()
        ...
        if FOVDrawing then FOVDrawing:Remove() FOVDrawing = nil end -- Очистка FOV
    end)
    
    print("DIX HUB V40.1 GUI SUCCESSFULLY LAUNCHED (Mobile Stable).")
else
...
print("DIX HUB V40.1 Core logic initialized.")
