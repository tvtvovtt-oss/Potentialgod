-- ====================================================================
-- [DIX] FINAL SCRIPT V32.0 (ADVANCED SILENT AIM FRAMEWORK)
-- Feature: Added Velocity Prediction and a Raycast Bypass structure.
-- !! WARNING: The bypass structure requires game-specific analysis. !!
-- ====================================================================
...
local ContextActionService = game:GetService("ContextActionService") -- NEW: Can be used for better M1 detection
local LocalPlayer = Players.LocalPlayer or Players.PlayerAdded:Wait() 
local Camera = Workspace.CurrentCamera 
local Drawing = pcall(function() return Drawing end) and Drawing or nil
local ReplicatedStorage = game:GetService("ReplicatedStorage") -- Added for RemoteEvent analysis
...
local Target_HumanoidRootPart = true 

local AimConnection = nil
local CurrentTarget = nil    

-- 4. Hitbox Settings (Logic preserved)
...
-- 5. ESP Settings (Glow ESP logic preserved)
...
-- ====================================================================
-- [HELPER: PREDICATION & BYPASS LOGIC] - NEW CORE
-- ====================================================================

-- Function to predict the target's position based on their velocity
local function GetPredictedPosition(TargetPart, BulletSpeed)
    local Velocity = TargetPart.AssemblyLinearVelocity
    local MyPosition = LocalPlayer.Character.PrimaryPart.Position
    local TargetPosition = TargetPart.Position
    local Distance = (MyPosition - TargetPosition).Magnitude
    
    local TimeToTarget = Distance / (BulletSpeed or 2000) -- 2000 is a safe default for Roblox bullet speed
    
    local PredictedPosition = TargetPosition + (Velocity * TimeToTarget)
    
    -- Safety check: don't predict too far
    if (PredictedPosition - TargetPosition).Magnitude > 20 then
        return TargetPosition -- Fallback if prediction is extreme
    end
...
-- (FindNearestTarget remains the same, using current position for FOV/Distance)
-- ====================================================================
...
-- (AimFunction, StartAiming, StopAiming logic remains the same)
local function AimFunction()
    ...
    -- Standard target finding for both Silent Aim and Standard Aimbot
    local TargetRootPart = nil
    -- ... (Target persistence and finding logic here)

    -- If Silent Aim is NOT enabled, move the camera
    if not IsSilentAimEnabled and TargetRootPart then
...
local function HandleSilentAimShot()
    ...
    if TargetRootPart then
        ...
        if AimPart and AimPart:IsA("BasePart") then
            -- 1. Get Predicted Position
            local PredictedPosition = GetPredictedPosition(AimPart, 1500) -- Use 1500 studs/s as default bullet speed
            
            print("[DIX] Silent Aim: Found Target. Predicted Pos: " .. tostring(PredictedPosition))
            
            -- 2. Apply Bypass (Raycast Hook)
            -- ЭТО НАИБОЛЕЕ РАСПРОСТРАНЕННЫЙ СПОСОБ, НО МОЖЕТ НЕ РАБОТАТЬ В КАЖДОЙ ИГРЕ!
            
            -- Проверяем, есть ли в твоем эксплойтере функция HookFunction
            if hookfunction and Workspace.Raycast and AimPart.CFrame then 
                IsBypassActive = true
                
                -- Сохраняем оригинальную функцию Raycast
                local originalRaycast = hookfunction(Workspace.Raycast, function(self, origin, direction, params)
                    ...
                    if IsSilentAimEnabled and IsBypassActive and AimPart.Parent and AimPart.CFrame then
                        ...
                        -- Подменяем направление рейкаста на цель
                        return originalRaycast(self, origin, TargetDirection, params)
            ...
            -- 3. Trigger the Shot (You must press M1 here for the hook to work!)
            -- Тут ничего не надо делать, так как хук будет работать, когда игра вызовет Raycast при твоем выстреле.
            
            -- 4. Clean up (Deactivate the bypass after a short moment)
            task.delay(0.1, function()
                IsBypassActive = false
            end)
            
            
            -- [[ ALTERNATIVE: REMOTE EVENT HOOK (IF GAME USES REMOTES) ]]
            -- Если ты знаешь RemoteEvent, который использует игра для стрельбы (например, "FireGun"), 
            -- то можно перехватить его:
            -- local FireRemote = ReplicatedStorage:FindFirstChild("FireGun") 
            -- if FireRemote and hookmetamethod then
            --    local originalFire = hookmetamethod(FireRemote, "__index", function(self, index)
            --        if index == "FireServer" and self == FireRemote then
            --            -- Здесь ты должен проверить аргументы (например, FireRemote:FireServer(direction, origin))
            --            -- и заменить 'direction' на TargetDirection
            --            -- return originalFire(self, TargetDirection, origin)
            --        end
            --        return originalFire(self, index) -- or originalFire(self)
            --    end)
            -- end
...
-- Мы используем ContextActionService для более надежной фиксации нажатия
...
-- ====================================================================
-- [GUI LOGIC] (Updated with Silent Aim logic)
-- ====================================================================

-- (All Hitbox and ESP logic remains the same)

if WindUi then
    local Window = WindUi:CreateWindow({
        Title = "DIX HUB V32.0",
        Author = "by Dixyi",
        Folder = "DIX_Hub_V32_Final",
    ...
    -- 1B. SILENT AIM ON/OFF Toggle (NEW)
    AimSection:Toggle({ 
        Flag = "SilentAimToggle", 
        Title = "Silent AIM: ON/OFF", 
        Desc = "Uses predicted position and bypass structure.", 
    ...
    Window:OnClose(function()
        ...
        ContextActionService:UnbindAction("SilentAimShot") -- Unbind the M1 hook on close
    end)
    
    -- Restore FOV visibility on window opening
    Window:OnOpen(function()
        if IsAimbotEnabled or IsSilentAimEnabled then 
    ...
    print("DIX HUB V32.0 launched. Advanced Silent Aim framework ready.")
end
