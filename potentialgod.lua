-- ====================================================================
-- [DIX] FINAL SCRIPT V30.5 (Maximum Compatibility Fix)
-- FIX: Ensured GUI is fully rendered with DUMMY defaults BEFORE loading
-- config and syncing. This maximizes compatibility with unstable exploits.
-- ====================================================================
...
if not WindUi then
    warn("[DIX CRITICAL ERROR] Failed to initialize WindUi. Aborting.")
    return
end
...
-- 3. Configuration Variables (Initial, hardcoded defaults for GUI creation)
local ConfigName = "DIX_HUB_V30_CONFIG"

-- Aimbot Settings
local IsAimbotEnabled = false  -- Default: OFF
...
-- ESP Settings 
local IsESPEnabled = false     -- Default: OFF
...
-- (Функции Hitbox/SaveConfig/LoadConfig/UpdateFOVVisual/Aimbot/ESP остаются теми же)

-- ====================================================================
-- [DIX CONFIG CORE] - Save & Load (Logic Retained)
-- ====================================================================
...
local function LoadConfig()
    ...
    if not loadedSettings then
        print("[DIX INFO] No saved configuration found. Using GUI defaults.")
        return
    end
...
if WindUi then
    ...
    -- ================================================================
    -- COMBAT SECTION 2: Target Part Selector
    -- ================================================================
    ...
    -- ***ФИНАЛЬНЫЙ ФИКС: Загрузка конфигурации и запуск логики ПОСЛЕ полной отрисовки GUI***
    LoadConfig() 
    Window:Sync() -- Синхронизация GUI с загруженными значениями
    if IsAimbotEnabled then StartAiming() end
    if IsESPEnabled then StartESP() end

    print("DIX HUB V30.5 launched. Hopefully resolved startup issue.")
end
